...
const Dogmas: React.FC = () => {
  ...
  const fetchDogmas = async (q?: string) => {
    setLoading(true);
    try {
  ...
  const groupedDogmas = useMemo(() => {
    ...
    // Ordenar grupos cronologicamente pelo ano do primeiro dogma do grupo
  ...
  const renderDogmaCard = (d: Dogma, idx: number) => (
    <article key={idx} className="bg-white p-10 md:p-12 rounded-[3.5rem] border border-stone-100 shadow-sm relative group hover:shadow-sacred/10 transition-all overflow-hidden animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="flex justify-between items-start mb-8">
         <div className="space-y-4 max-w-[85%]">
            ...
            <div className="flex items-center gap-4 flex-wrap">
               <h3 className="text-4xl font-serif font-bold text-stone-900 leading-tight tracking-tight">{d.title}</h3>
               {d.sourceUrl && (
                 <a 
                   href={d.sourceUrl} 
                   target="_blank" 
                   rel="noopener noreferrer" 
                   className="p-3 bg-[#fcf8e8] text-[#8b0000] rounded-full hover:bg-[#8b0000] hover:text-white transition-all shadow-md active:scale-95 group/link"
      ...
      <div className="space-y-6 relative z-10">
        <div className="bg-stone-50/50 p-8 rounded-[2rem] border border-stone-100/50">
          <p className="text-stone-700 font-serif italic text-2xl leading-relaxed tracking-tight">
            "{d.definition}"
          </p>
        </div>
      </div>

      <div className="flex flex-wrap items-center justify-between gap-6 pt-8 border-t border-stone-50 mt-8">
        <div className="flex gap-2 flex-wrap">
           {d.tags.map((tag, tIdx) => (
             <span key={tIdx} className="text-[9px] font-black uppercase tracking-tighter text-stone-400 bg-stone-50 px-3 py-1.5 rounded-lg border border-stone-100/50">
               #{tag}
             </span>
           ))}
        </div>
        {d.period && (
          <span className="text-[10px] font-black text-[#d4af37] uppercase tracking-[0.2em] px-5 py-2 bg-[#1a1a1a] rounded-full shadow-lg">
            {d.period}
          </span>
        )}
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-16 animate-in fade-in duration-1000 pb-32">
      <header className="text-center space-y-8">
        ...
        <h2 className="text-7xl font-serif font-bold text-stone-900 tracking-tight text-shadow-sacred">Verdades de Fé</h2>
      ...
      {/* Seção Santuário Mariano */}
      {filterTag === 'marian' && (
        <section className="bg-[#fcf8e8] p-12 rounded-[4rem] border border-[#d4af37]/20 shadow-xl animate-in slide-in-from-top-10 duration-700 relative overflow-hidden group">
           <div className="absolute -top-10 -right-10 opacity-[0.05] pointer-events-none group-hover:scale-125 transition-transform duration-1000">
              <Icons.Cross className="w-64 h-64 text-[#8b0000]" />
           </div>
           <div className="flex items-center gap-8 mb-8">
              <div className="p-6 bg-white rounded-3xl shadow-sacred">
                <Icons.Cross className="w-12 h-12 text-[#8b0000]" />
              </div>
              <div>
                 <h3 className="text-5xl font-serif font-bold text-stone-900 tracking-tight">Santuário Mariano</h3>
                 <p className="text-[11px] font-black uppercase tracking-[0.6em] text-[#d4af37] mt-2">Colunas da Mariologia Dogmática</p>
              </div>
           </div>
           <p className="text-2xl text-stone-700 font-serif italic leading-relaxed max-w-4xl relative z-10">
             A Igreja definiu quatro verdades fundamentais sobre a Virgem Maria: sua Maternidade Divina, Virgindade Perpétua, Imaculada Conceição e Assunção. Abaixo, explore as definições que iluminam o papel de Maria na História da Salvação.
           </p>
        </section>
      )}

      {/* Controles Profissionais: Busca e Agrupamento */}
      <section className="bg-white p-10 md:p-14 rounded-[4.5rem] shadow-3xl border border-stone-100 space-y-12">
        <div className="flex flex-col lg:flex-row gap-10 items-center justify-between">
           <div className="flex flex-wrap justify-center lg:justify-start gap-3">
              {[
                ...
                <button
                  ...
                  className={`px-8 py-4 rounded-full text-[11px] font-black uppercase tracking-[0.2em] transition-all border ${filterTag === tag.id ? 'bg-[#8b0000] text-white border-[#8b0000] shadow-sacred scale-105' : 'bg-white text-stone-400 border-stone-100 hover:border-[#d4af37]'}`}
                >
                  {tag.label}
                </button>
              ))}
           ...
           <div className="flex items-center gap-2 bg-stone-50 p-2 rounded-full border border-stone-100">
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-3 px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === mode.id ? 'bg-white text-stone-900 shadow-md scale-105' : 'text-stone-300 hover:text-stone-500'}`}
                >
        ...
        <form onSubmit={(e) => { e.preventDefault(); fetchDogmas(query); }} className="relative group">
          ...
          <input 
            ...
            className="w-full pl-24 pr-12 py-9 bg-stone-50 border border-stone-200 rounded-[3rem] focus:ring-16 focus:ring-[#d4af37]/5 outline-none text-2xl md:text-3xl font-serif italic shadow-inner transition-all placeholder:text-stone-200"
          />
      ...
      <div className="pb-40">
        {loading ? (
          <div className="grid gap-12">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-64 bg-white rounded-[4rem] animate-pulse border border-stone-100 shadow-sm" />
            ))}
          </div>
                 ...
                 return (
                   <section key={gIdx} className="space-y-10 animate-in fade-in duration-700">
                      <button 
                        ...
                        className={`w-full flex items-center gap-12 group p-10 rounded-[4rem] transition-all border ${isExpanded ? 'bg-stone-50/50 border-stone-200 shadow-inner' : 'bg-white border-stone-100 hover:border-[#d4af37]/40 shadow-xl'}`}
                      >
                         ...
                         <div className="flex flex-col items-center flex-shrink-0">
                            ...
                            <div className="flex items-center gap-6">
                               <h4 className="text-4xl md:text-5xl font-serif font-bold text-stone-900 tracking-tight leading-none">{groupName}</h4>
                               <div className={`p-4 bg-white rounded-full border border-stone-100 shadow-sm transition-transform duration-700 ${isExpanded ? 'rotate-180' : ''}`}>
          ...
          <div className="h-96 flex flex-col items-center justify-center text-center p-20 bg-white/40 rounded-[5rem] border-2 border-dashed border-stone-200 shadow-inner">
             <div className="p-8 bg-white rounded-full shadow-2xl mb-10 rotate-12">

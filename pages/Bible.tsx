...
const Bible: React.FC = () => {
  ...
  const [fontSize, setFontSize] = useState(1.15); // rem base para mobile
  ...
  const loadContent = useCallback(async (bookName: string, chapter: number) => {
    ...
    try {
      ...
      if (staticData.length > 0) {
        ...
        setVerses([{ book: bookName, chapter, verse: 1, text: "Conteúdo indisponível offline. Por favor, conecte-se para baixar." }]);
      }
  ...
  const LibraryView = () => (
    <div className="space-y-10 md:space-y-16 animate-in fade-in duration-700 pb-20 px-2">
      <header className="text-center space-y-6 md:space-y-8 pt-6 md:pt-10">
        <div className="flex justify-center">
           <div className="p-5 md:p-8 bg-white dark:bg-stone-900 rounded-[2rem] md:rounded-[2.5rem] shadow-sacred border border-gold/30">
        ...
        <div className="px-4">
          ...
          <p className="text-stone-400 italic text-lg md:text-2xl font-serif mt-2">"O Verbo se fez carne."</p>
        </div>
            ...
            <div className="grid gap-12 md:gap-16">
              {Object.entries(categories as any).map(([category, books]) => (
                <div key={category} className="space-y-6 md:space-y-8">
                  <h4 className="px-4 text-[9px] font-black uppercase tracking-[0.4em] text-sacred dark:text-gold/60 flex items-center gap-3">
                    ...
                    {(books as string[]).map(bookName => {
                      ...
                      return (
                        <button 
                          ...
                          className={`p-4 md:p-6 rounded-[1.5rem] md:rounded-[2.5rem] border shadow-md md:shadow-lg transition-all text-left group relative overflow-hidden active:scale-95 flex flex-col justify-between h-24 md:h-32 ${isOffline ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-100 dark:border-emerald-900/30' : 'bg-white dark:bg-stone-900 border-stone-100 dark:border-stone-800'}`}
                        >
                          <div className="relative z-10">
                             <h5 className="text-sm md:text-lg font-serif font-bold text-stone-800 dark:text-stone-100 group-hover:text-gold leading-tight truncate">{bookName}</h5>
                          ...
                          <div className="flex justify-end items-center mt-1">
                             {isOffline ? <Icons.Pin className="w-3 h-3 text-emerald-500" /> : <Icons.ArrowDown className="w-3 h-3 -rotate-90 text-stone-200 opacity-0 group-hover:opacity-100" />}
                          </div>
                        </button>
                      );
                    })}
  ...
  const ChapterSelector = () => (
    <div className="space-y-10 md:space-y-12 animate-in slide-in-from-bottom-8 duration-700 pb-32 px-4">
  ...
  const ReadingView = () => (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-700 pb-40 px-2 md:px-0">
      <nav className="sticky top-2 md:top-4 z-[200] bg-white/95 dark:bg-stone-950/95 backdrop-blur-xl rounded-full md:rounded-[2.5rem] border border-stone-200 dark:border-white/10 shadow-2xl p-2 md:p-3 flex items-center justify-between">
         ...
         <div className="flex items-center gap-2 md:gap-4 px-2 md:px-6">
            <button onClick={() => setFontSize(Math.max(0.9, fontSize - 0.1))} className="text-stone-400 hover:text-gold transition-colors font-serif text-sm md:text-lg">A-</button>
            <button onClick={() => setFontSize(Math.min(2.2, fontSize + 0.1))} className="text-stone-400 hover:text-gold transition-colors font-serif text-lg md:text-2xl">A+</button>
         </div>

         <div className="hidden sm:flex pr-2">
            <span className={`px-4 py-2 rounded-xl text-[8px] font-black uppercase tracking-widest flex items-center gap-2 ${sourceType === 'local' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-gold/10 text-gold'}`}>
               <Icons.Pin className="w-3 h-3" /> {sourceType === 'local' ? 'Offline' : 'Sync'}
            </span>
         </div>
      </nav>
      ...
      <div className="max-w-4xl mx-auto">
        <article 
          className="parchment dark:bg-stone-900 p-6 md:p-20 rounded-[2.5rem] md:rounded-[4rem] shadow-xl border border-stone-100 dark:border-white/5 relative min-h-[60vh]"
        ...
        >
           {loading ? (
              <div className="py-32 text-center space-y-6 animate-pulse">
                ...
                <p className="text-xl font-serif italic text-stone-400">Consultando Escrituras...</p>
              </div>
           ) : (
             <div className="space-y-4 md:space-y-6 relative z-10 text-stone-800 dark:text-stone-200">
                {verses.map((v, i) => (
                  <span key={i} className="inline group break-words">
                    <sup className="text-[0.65em] font-black text-sacred dark:text-gold mr-1.5 opacity-70 group-hover:opacity-100">{v.verse}</sup>
           ...
           <div className="mt-10 pt-10 border-t border-stone-100 dark:border-stone-800 flex justify-center">
              <Icons.Cross className="w-6 h-6 opacity-10" />
           </div>
        </article>
      </div>

      {/* Navegação fixa na base para Mobile */}
      <div className="flex justify-center gap-4 md:gap-6 pb-10 mt-4">
         <button 
           disabled={selectedChapter <= 1}
           onClick={() => navigateChapter(-1)}
           className="flex-1 max-w-[150px] py-4 md:py-6 bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 rounded-full md:rounded-[2rem] font-black uppercase text-[9px] md:text-[10px] tracking-[0.3em] disabled:opacity-20 transition-all hover:border-gold shadow-md"
         >
           Anterior
         </button>
         <button 
           ...
           onClick={() => navigateChapter(1)}
           className="flex-1 max-w-[200px] py-4 md:py-6 bg-gold text-stone-900 rounded-full md:rounded-[2rem] font-black uppercase text-[9px] md:text-[10px] tracking-[0.3em] shadow-lg hover:scale-105 active:scale-95 transition-all"
         >
           Próximo
         </button>
      </div>

...
const DailyLiturgy: React.FC = () => {
  ...
  const playAudio = async () => {
    ...
    try {
      const textToRead = activeTab === 'readings' 
        ...
        : `Reflexão teológica. ${data.gospel?.reflection || data.gospel?.homily || 'Reflexão não disponível.'}`;
  ...
  return (
    <div className="space-y-10 max-w-5xl mx-auto pb-40 px-4 animate-in fade-in duration-700 relative">
      {/* BOTÕES DE NAVEGAÇÃO LATERAL (SIDE GUTTERS) - ESTILO BÍBLIA */}
      <div className="fixed inset-y-0 left-0 z-[100] flex items-center px-2 md:px-8 pointer-events-none">
         <button 
            onClick={() => navigateDate(-1)}
            className="p-5 md:p-7 bg-white/20 dark:bg-black/20 backdrop-blur-xl rounded-full text-gold pointer-events-auto transition-all shadow-2xl border border-gold/10 opacity-30 hover:opacity-100 hover:scale-110 active:scale-90 shadow-sacred/10"
      ...
      <div className="fixed inset-y-0 right-0 z-[100] flex items-center px-2 md:px-8 pointer-events-none">
         <button 
            onClick={() => navigateDate(1)}
            className="p-5 md:p-7 bg-white/20 dark:bg-black/20 backdrop-blur-xl rounded-full text-gold pointer-events-auto transition-all shadow-2xl border border-gold/10 opacity-30 hover:opacity-100 hover:scale-110 active:scale-90 shadow-sacred/10"
      ...
      {loading || rendering ? (
        <div className="py-60 text-center space-y-12">
          ...
          <div className="space-y-4">
            <p className="text-3xl font-serif italic text-stone-900 dark:text-stone-100">Consultando o Nártex...</p>
            <p className="text-stone-400 font-serif italic animate-pulse">Buscando o Alimento da Palavra nos arquivos sagrados</p>
          </div>
        </div>
      ) : error ? (
        <div className="py-40 text-center space-y-8 animate-in zoom-in-95">
           <div className="p-10 bg-white dark:bg-stone-900 rounded-[3rem] border-2 border-dashed border-stone-200 dark:border-stone-800 max-w-md mx-auto space-y-6">
              <Icons.History className="w-16 h-16 text-stone-300 mx-auto" />
              <div className="space-y-2">
                 <h3 className="text-2xl font-serif font-bold text-stone-900 dark:text-stone-100">Ops! Ocorreu um erro</h3>
                 <p className="text-stone-400 font-serif italic text-lg">Não conseguimos conectar com a inteligência litúrgica no momento.</p>
              </div>
              <button 
                onClick={() => loadLiturgy(date)}
                className="w-full py-4 bg-gold text-stone-900 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg hover:scale-105 active:scale-95 transition-all"
              >
                Tentar Novamente
              </button>
           </div>
        ...
        <div className="space-y-12 animate-in fade-in duration-1000">
          ...
          <header className={`bg-white dark:bg-stone-900 p-10 md:p-20 rounded-[4rem] border-t-[20px] ${theme.border} shadow-2xl text-center relative overflow-hidden`}>
            ...
            <div className="relative z-10 space-y-6">
              ...
              <p className="text-stone-400 text-xl font-serif italic py-4">{formattedDate}</p>
            </div>
          </header>
          ...
          {activeTab === 'readings' ? (
            <article className="parchment dark:bg-stone-900/40 p-10 md:p-24 rounded-[3.5rem] md:rounded-[5rem] shadow-xl border border-stone-100 dark:border-stone-800 space-y-24" style={{ fontSize: `${fontSize}rem` }}>
              ...
              <section className={`${theme.bg} p-12 md:p-20 rounded-[3.5rem] border-l-[16px] ${theme.border} space-y-10 shadow-inner`}>
            ...
            <article className="animate-in fade-in slide-in-from-right-8 duration-700">
               <div className="bg-white dark:bg-stone-900 p-12 md:p-24 rounded-[4rem] shadow-4xl border border-gold/10 space-y-10 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-center gap-8 py-20">
            <button 
              onClick={() => navigateDate(-1)}
            ...
            >
              <Icons.ArrowDown className="w-5 h-5 rotate-90 group-hover:-translate-x-3 transition-transform" /> Dia Anterior
            </button>
            <button 
              onClick={() => navigateDate(1)}
            ...
            >
              Dia Seguinte <Icons.ArrowDown className="w-5 h-5 -rotate-90 group-hover:translate-x-3 transition-transform" />
            </button>
          </div>
        </div>
      )}
      ...
      <div className="fixed bottom-24 right-6 md:right-12 flex flex-col gap-4 z-[300]">
         ...
         <button 
            onClick={playAudio} 
            disabled={loading || rendering || !data}

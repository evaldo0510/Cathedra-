      ...
      <header className="text-center space-y-8">
        ...
        <div className="max-w-3xl mx-auto px-4 space-y-6">
          <div className="relative group">
            ...
            <input 
              type="text"
              placeholder="Buscar por nome ou causa (ex: Santa Rita, Música...)"
              value={searchTerm}
      ...
      {selectedSaint && (
        <div className="fixed inset-0 z-[500] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in" onClick={() => setSelectedSaint(null)}>
          <div className="bg-white dark:bg-stone-900 w-full max-w-4xl max-h-[90vh] rounded-[4rem] shadow-4xl overflow-hidden flex flex-col border border-stone-100 dark:border-stone-800 animate-modal-zoom relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-8 md:p-16 flex-1 overflow-y-auto custom-scrollbar space-y-12 -mt-16 relative z-10">
              <header className="space-y-6 text-center md:text-left">
                 ...
                 <h2 className="text-5xl md:text-8xl font-serif font-bold text-stone-900 dark:text-stone-100 tracking-tighter leading-none">{selectedSaint.name}</h2>
                 <p className="text-sacred dark:text-gold text-xl md:text-3xl font-serif italic border-l-4 border-gold/30 pl-6">{selectedSaint.patronage}</p>
              </header>

              {selectedSaint.quote && (
                <div className="bg-[#fcf8e8] dark:bg-stone-800/50 p-10 rounded-[3rem] border-l-[12px] border-gold shadow-inner">
                   <p className="text-2xl md:text-4xl font-serif italic text-stone-800 dark:text-stone-100 leading-snug tracking-tight">
                     "{selectedSaint.quote}"
                   </p>
                </div>
              )}
              ...
              <div className="prose dark:prose-invert max-w-none">
                 <p className="text-lg md:text-2xl font-serif text-stone-700 dark:text-stone-300 leading-relaxed text-justify whitespace-pre-wrap">
